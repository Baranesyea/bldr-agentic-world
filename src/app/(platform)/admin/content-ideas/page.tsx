...
const STORAGE_KEY = "bldr_content_ideas";
const NEXT_ID_KEY = "bldr_content_ideas_next_id";

function getIdeas(): ContentIdea[] {
...
function formatId(n: number): string {
  return `CONTENT-${String(n).padStart(3, "0")}`;
}
...
export default function ContentIdeasPage() {
  ...
  const persist = useCallback((updated: ContentIdea[]) => {
    setIdeas(updated);
    saveIdeas(updated);
  }, []);
  ...
  const submitAll = () => {
    ...
    const newIdeas: ContentIdea[] = valid.map((r) => {
  ...
  return (
    <div style={{ padding: "32px", maxWidth: "1100px", margin: "0 auto" }}>
      ...
      <p style={{ color: "rgba(240,240,245,0.6)", marginBottom: "28px", fontSize: "14px" }}>
        ניהול רעיונות לתכנים — כותרות, תיאורים ונושאים
      </p>
