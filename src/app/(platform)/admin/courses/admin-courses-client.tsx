  ...
  const duplicateCourse = async (course: Course) => {
    try {
      const res = await fetch("/api/courses", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "duplicate", id: course.id }),
      });
      if (res.ok) {
        const { course: dup } = await res.json();
        // Refetch to get properly serialized data
        const listRes = await fetch("/api/courses");
        if (listRes.ok) {
          const { courses: refreshed } = await listRes.json();
          setCourses(refreshed.map(serializeCourse));
        } else {
          // Fallback: add the duplicate locally
          setCourses((prev) => [...prev, serializeCourse(dup)]);
        }
      }
    } catch (e) {
      console.error("Failed to duplicate course:", e);
    }
  };
  ...
  const saveOrder = async (newCourses: Course[]) => {
    ...
    try {
      await fetch("/api/courses", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "reorder", courseIds }),
      });
    } catch (e) {
...
// Helper to normalize course data from API
function serializeCourse(c: Record<string, unknown>): Course {
  const course = c as Record<string, unknown>;
  return {
    id: (course.id as string) || "",
    title: (course.title as string) || "",
    description: (course.description as string) || "",
    status: (course.status as "draft" | "active" | "coming_soon") || "draft",
    featured: false,
    thumbnailUrl: (course.thumbnail as string) || (course.thumbnailUrl as string) || "",
    createdAt: course.createdAt ? String(course.createdAt) : new Date().toISOString(),
    updatedAt: course.updatedAt ? String(course.updatedAt) : new Date().toISOString(),
    chapters: Array.isArray(course.chapters)
      ? (course.chapters as Record<string, unknown>[]).map((ch, ci) => ({
          id: (ch.id as string) || "",
          number: ci + 1,
          title: (ch.title as string) || "",
          lessons: Array.isArray(ch.lessons)
            ? (ch.lessons as Record<string, unknown>[]).map((l, li) => ({
                id: (l.id as string) || "",
                number: li + 1,
                ...
                skills: [],
                hasAssignment: (l.hasAssignment as boolean) || false,
                assignmentText: "",
