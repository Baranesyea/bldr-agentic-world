...
const STORAGE_KEY = "bldr_dev_ideas";
const NEXT_ID_KEY = "bldr_dev_ideas_next_id";

function getIdeas(): DevIdea[] {
...
function saveIdeas(ideas: DevIdea[]) {
...
function formatId(n: number): string {
  return `IDEA-${String(n).padStart(3, "0")}`;
}
...
export default function IdeasPage() {
  ...
  const persist = useCallback((updated: DevIdea[]) => {
    setIdeas(updated);
    saveIdeas(updated);
  }, []);
  ...
  const submitAll = () => {
    ...
    const newIdeas: DevIdea[] = valid.map((r) => {
      const idea: DevIdea = {
        id: formatId(nextId),
        title: r.title.trim(),
